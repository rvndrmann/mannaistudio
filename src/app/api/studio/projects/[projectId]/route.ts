...
export async function GET(request: NextRequest, { params }: { params: Promise<{ projectId: string }> }) {
  try { const { projectId } = await params; const { supabase, user, project } = await ownedProject(projectId)
    ...
    // Started here, awaited after the batch below. It mutates the shot rows in
    // place so the response carries the repair, but nothing in that batch reads
    // them — so the repair and the reads overlap instead of queueing.
    const locationRepair = shots?.length && entities?.length
      ? ensureShotLocations(supabase, { shots, entities: entities as { id: string; type: string }[] })
    ...
    const activeSessionId = chatSessions?.find((session) => session.id === requestedSessionId)?.id || chatSessions?.[0]?.id
    // Everything left, in one batch.
    //
    // These ran as six sequential phases — messages and proposals, then runs,
    // then the job list, then the whole-project spend, then seven more for the
    // production panels — and none of them needs anything another returns. On a
    // project with real history the endpoint took 33 seconds, which is what the
    // workspace waits on after every approval and every generation, and is most
    // of why the studio feels stuck rather than slow.
    const wantsProduction = Boolean(features.series_hierarchy_enabled || features.continuity_checks_enabled || features.generation_jobs_enabled)
    const [
      { data: chatMessages },
      { data: actionProposals },
      { data: workflowRuns },
      { data: baseGenerationJobs },
      { data: spendRows },
      productionRows,
    ] = await Promise.all([
      ...
      supabase.from("creator_generation_jobs").select("type,status,estimated_credits,credits_used,credits_refunded,episode_id,billing_mode").eq("project_id", projectId),
      wantsProduction
        ? Promise.all([
    ...
    await Promise.all([
      locationRepair,
      failAbandonedRuns(supabase, { projectId, userId: user.id, runs: workflowRuns || [] }).catch((error) => console.warn("Could not close abandoned Director runs:", error)),
    ])
    ...
    if (productionRows) {
      const [{ data: series }, { data: scenes }, { data: referenceAssets }, { data: continuityIssues }, { data: revisions }, { data: generationJobs }, { data: creditAccount }] = productionRows
