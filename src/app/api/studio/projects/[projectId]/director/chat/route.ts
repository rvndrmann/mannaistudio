...
async function generateBulkEntityReferenceImages(
  ...
  for (let offset = 0; offset < requested.length; offset += batchSize) {
    ...
    const results = await Promise.allSettled(batch.map(async (entity) => {
      ...
      // Reference art the Director generates is shot on the same camera
      // package as reference art generated by hand, or the library the
      // storyboard matches against would be optically inconsistent with it.
      // The base prompt is what gets stored; only the model sees the clause.
      const cameraSettings = resolveCameraSettings({
        block: cameraBlockForEntityType(entity.type),
        projectDefaults: projectCameraDefaults(input.context.project),
      })
      const framedPrompt = applyCameraSettings(prompt, cameraSettings, { opticsOnly: Boolean(styleDna) })
