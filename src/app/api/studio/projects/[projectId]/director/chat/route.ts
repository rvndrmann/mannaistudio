...
async function maybeHandleShotImageBatch(input: WorkflowRequestInput, normalized: string) {
  ...
  const prompts = Object.fromEntries(selected.map((shot) => {
    ...
    return [String(numberOf(shot)), [
      ...
      ...composeLookDirectives(style, styleDna, "shot"),
      mentionContext,
    ].filter(Boolean).join("\n\n")]
  }))
...
async function maybeHandleWorkflowRequest(input: WorkflowRequestInput) {
  ...
  if (!forbidsAllMediaGeneration && !forbidsImageGenerationRequest && (namesImage || wantsShotRedo)) {
    ...
    const resolvedPrompt = [stripIdentityDescriptions(prompt), `Required composition: ${aspectRatio}.`, ...composeLookDirectives(style, projectStyleDna(input.context.project), "shot"), mentionContext].filter(Boolean).join("\n\n")
...
async function generateBulkEntityReferenceImages(
  ...
  for (let offset = 0; offset < requested.length; offset += batchSize) {
    ...
    const results = await Promise.allSettled(batch.map(async (entity) => {
      const prompt = buildEntityReferenceImagePrompt(entity as MentionableEntity, style, composeLookDirectives(style, styleDna, styleBlockForEntityType(entity.type)))
      // Only a project that has chosen a camera package gets one here. The
      // Director has no per-image switch to read, so with no project package
      // set it generates the reference exactly as the prompt describes it —
      // the same promise the panels make when their switch is off.
      const projectCamera = projectCameraDefaults(input.context.project)
      const cameraSettings = projectCamera
        ? resolveCameraSettings({ block: cameraBlockForEntityType(entity.type), projectDefaults: projectCamera })
        : null
      const framedPrompt = cameraSettings
        ? applyCameraSettings(prompt, cameraSettings, { opticsOnly: Boolean(styleDna) })
        : prompt
