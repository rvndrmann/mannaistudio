...
const toolDescriptions: Record<DirectorToolName, string> = {
  ...
  submit_generation: "Propose image or video generation jobs. Calling this tool is how the approval card is created — it spends nothing on its own, and the user cannot approve a generation you have not proposed, so call it rather than waiting to be told to. When the user names shots or scenes by number, pass those numbers as `request.shotNumbers` with `request.episodeId` and key `prompts` by the same numbers; the server resolves them to the correct shots. Use `request.shotIds` only for ids you read from a tool result. Include only the shots the user actually asked for, and never widen the request to neighbouring shots or to every shot missing media. For an image, do not attach the shot's existing keyframe as a reference: a regenerate should build from the entity references and the prompt, not re-derive the picture it is replacing, and `request.useExistingFrame` is set to true only when the user asks to keep the current composition, framing, or layout. A video needs none of this — the shot's approved keyframe is the clip's first frame and is attached for you, so never pass it yourself and never ask the user to pick one. Before submitting, check the prompt's @mentions against `list_production_entities`: if one names an asset the project does not have, do not submit with the dangling reference. Offer to create it with `create_production_entity` and generate its reference image, using existing characters and assets as visual reference, then submit once it exists — or ask the user which existing asset to use instead. A shot that already has a job running is not a reason to refuse: the user asking again is the user asking for another take. Say in one line that a render is already in flight, then propose the new one anyway so the approval card exists and they can decide — an immutable result is saved per job, so a second take never overwrites the first. Only skip the proposal when the user explicitly says they want to wait for the running job.",
  ...
  accept_existing_art: "Keep the art a character, asset, or shot already has, recording it as matching the current description. Generates nothing and costs nothing. Use this when the user says the existing images are good enough.",
