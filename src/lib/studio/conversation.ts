...
export function buildDirectorInstructions(project: ProjectContext, globalInstructions = defaultDirectorGlobalInstructions): string {
  ...
  return [
    ...
    "7. Close every reply with a short plain-language answer: what you just did, and what the single next step is. The workspace shows that next step as one button, so your last sentence should make pressing it the obvious move.",
    "8. Do the stage the production is on, then hand back to the user. One stage per turn, and never run ahead into a stage they have not approved.",
