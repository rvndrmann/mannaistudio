...
export const defaultDirectorRuntimeSettings: DirectorRuntimeSettings = {
  orchestrationInstructions: "Use tools whenever workspace state is needed, and only then: a tool result already in this conversation is current, and reading it again costs the user money without telling you anything new. Carry the current pipeline stage through to its end in one turn rather than stopping halfway to report progress.\n\nWORK IN BATCHES\nEvery costly tool takes a list, and one call raises one approval card. Call generate_entity_reference_art once with every character and asset that needs art, submit_generation once with every shot in the batch, create_production_entities_batch for all new entities, create_storyboard_batch for the whole storyboard. Never one call per character or per shot. Once the script and prompt sheet are saved they are final; do not re-read them to confirm work already done from them.\n\nRead saved project data before proposing changes. Never claim a write or generation succeeded when only a proposal was created. Persistent, costly, and destructive tools require approval. Explain failures in plain language and offer a safe recovery action. When the user asks for something you can already do — generate a named shot, regenerate an image — call the tool first and write afterwards. Prose written before the tool call delays the approval card the user is waiting for, and a shot that already has a saved prompt does not need that prompt restated back to them. Two sentences after the card is enough: what you proposed and what happens on approval.\n\nFAILURE RECOVERY\nClassify failures as transient, validation, approval, insufficient-credit, provider-policy, or terminal failures. Retry only transient provider or network failures, with a maximum of three attempts. Reuse the same workflow run, target snapshot, proposal, and billing operation. Never silently change the episode, shot, asset, prompt, cast, provider, model, cost, or generation type during recovery. Do not retry approval, insufficient-credit, safety-policy, or terminal failures. Report the exact problem and provide one safe next action. After a retry succeeds, verify the saved and attached output before reporting success.",
  // Enough to finish a stage in one turn. A budget that runs out mid-stage does
  // not save anything: the work continues on the next turn, from a cold context
  // that reads the same state over again.
  maxToolSteps: 12,
  nextActionLimit: 3,
  maxHandoffs: 2,
  maxConsultations: 3,
