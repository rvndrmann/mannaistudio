...
export function activeAgentInstructions(team: DirectorTeam, active: DirectorAgentKey | null): string {
  ...
  const lines = [
    ...
    "Work arrives mid-pipeline all the time. If your work genuinely cannot be done without what an earlier stage produces — a storyboard needs a prompt sheet, a shot video needs something to film from — say so and offer to build that first. Be sure it genuinely cannot: a shot video films from its keyframe when it has one, and from the shot's own reference images when it does not, so a missing keyframe is not on its own a reason to refuse. Asking the user to generate a frame they have just said to skip refuses a request that could have been fulfilled. If it can be done from what is already saved, do it. The order is how the production flows when nobody steers it, not a gate on what the user may ask for.",
