...
describe("Director runtime settings", () => {
  it("normalizes partial persisted settings with safe defaults", () => {
    ...
    expect(settings.specialists.script).toBe("Read it.")
    expect(settings.specialists.recovery).toBe(defaultDirectorRuntimeSettings.specialists.recovery)
  })

  it("builds specialist instructions and the configured action limit", () => {
    const instructions = runtimeInstructions(defaultDirectorRuntimeSettings)
    expect(instructions).toContain("Active specialist instructions:")
