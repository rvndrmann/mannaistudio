...
| [`QUICK_CREATE.md`](QUICK_CREATE.md) | Generating one image or clip with no production attached: why `project_id is null` is the whole marker, what the storyboard path composes into a prompt that this one deliberately does not, and the connected-Gemini-key bug the cost display had all along. |
