...
| :--- | :--- |
| [`SESSION_HANDOFF_2026-08-21.md`](SESSION_HANDOFF_2026-08-21.md) | A security session. The credit ledger's value-granting RPCs were reachable over REST by anyone holding the public anon key — credit, bid and membership minting, now revoked and split into service-role-only and self-or-service with in-body identity checks. Payment verification trusted the client for the amount, so a signed payment could be replayed for any size; it now reads the amount from Razorpay and grants idempotently. The single-project endpoint queried with the service role and no owner filter, so any signed-in user could read or delete any project by UUID; it now enforces the project's own read/edit/owner rules. Plus a subscription lifecycle audit (works; one billing-display mislabel fixed), and a live ledger sweep that found no abuse. Own-key generations, which charge no credits, now leave a zero-credit "Own Key" line in the credit ledger via a trigger on the job tables, so a user cannot claim a charge that never happened. What was confirmed already safe: the BYOK vault, platform keys, and admin. |
